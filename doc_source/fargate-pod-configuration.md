...
When provisioned, each pod running on Fargate receives 20 GB of container image layer storage\. Pod storage is ephemeral\. After a pod stops, the storage is deleted\. New pods launched onto Fargate after 5/28/2020 have encryption of the ephemeral storage volume enabled by default. The ephemeral pod storage is encrypted with industry-standard AES-256 algorithm using AWS Fargate-managed keys.
